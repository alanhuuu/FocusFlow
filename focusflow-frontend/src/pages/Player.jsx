...
export default function Player() {
  ...
  const [progress] = useState(35); // still fake (optional)
  ...
  // MusicKit
  // -------------------------
  const { music, ready, authorize, connected, fetchPlaylists, playPlaylist } =
    useMusicKit();
  ...
  useEffect(() => {
    ...
    const updateNowPlaying = () => {
      try {
        ...
        console.log("Now playing update error:", e);
      }
    };
  ...
  async function handlePlayPause() {
    ...
    try {
      ...
      // if nothing queued → user must select playlist
  ...
  async function handleShuffle() {
    if (!music) return;
    try {
      music.player.shuffleMode = music.player.shuffleMode === 0 ? 1 : 0;
    } catch (err) {
  ...
  async function handleRepeat() {
    if (!music) return;
    try {
      music.player.repeatMode = music.player.repeatMode === 0 ? 2 : 0;
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setEegData((prev) => {
        ...
        const newTbr = Math.max(
          0.5,
          Math.min(5, lastTbr + (Math.random() - 0.52) * 0.4)
        );
        ...
        return [...prev.slice(-49), { time: prev.length, tbr: newTbr }];
      });
    }, 200);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden text-white pb-28">
      {/* Background Image */}
      <div
      ...
      <div className="absolute top-8 left-10 z-20">
        <div className="text-4xl tracking-tight">focusflow</div>
      </div>
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20">
        ...
        >
          ...
          <div className="flex items-center justify-between mt-5 pt-4 border-t border-white/10">
            ...
            <div className="text-white/40 text-xs">Live EEG Stream</div>
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-20">
        <div
          className="flex items-center gap-5 rounded-2xl px-5 py-3 border border-white/10"
          style={{ backgroundColor: "#2f2546" }}
        >
          {/* Album Cover */}
          <img
            src={trackArtwork || albumCover}
            alt="Album cover"
          ...
          <div className="min-w-[180px]">
            ...
            <div className="text-white/60 text-xs truncate max-w-[180px]">
              {trackSubtitle}
            </div>
          </div>

          {/* Progress Bar (still fake for now) */}
        ...
        <div className="flex gap-2 items-start">
          ...
          <div className="flex flex-col items-center">
            ...
            >
              <button
                ...
                style={{
                  backgroundColor:
                    activeScene === "ambient" ? "#7532ff" : "transparent",
                }}
              >
                <LotusIcon />
              </button>
              <button
                ...
                style={{
                  backgroundColor:
                    activeScene === "home" ? "#7532ff" : "transparent",
                }}
              >
                <HomeIcon />
              </button>
              <button
                ...
                style={{
                  backgroundColor:
                    activeScene === "focus" ? "#7532ff" : "transparent",
                }}
              >
                <LightbulbIcon />
              </button>
          ...
          <select
            ...
            onChange={(e) => {
              ...
              if (id) {
                playPlaylist(id); // ✅ auto play on select
              }
            }}
            className="h-10 px-4 rounded-lg border border-white/10 text-white outline-none"
            style={{ backgroundColor: "#2f2546" }}
            disabled={!connected}
            title="Select Playlist"
          >
            <option value="">
              {connected ? "Select Playlist" : "Connect first"}
            </option>
          ...
          <button
            ...
            className="h-10 px-4 rounded-lg flex items-center justify-center hover:opacity-80 transition disabled:opacity-40 disabled:cursor-not-allowed"
            style={{ backgroundColor: "#7532ff" }}
          >
